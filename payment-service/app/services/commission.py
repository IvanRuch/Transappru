...
class CommissionService:
    ...
    async def update_rate(
        ...
        current_rate = await CommissionRate.filter(
            service_type=service_type, valid_to__isnull=True
        ).first()
        ...
        if current_rate:
            new_kazna_percent = (
                float(current_rate.kazna_percent)
                if kazna_percent is None
                else kazna_percent
            )
            new_kazna_min = (
                current_rate.kazna_min_amount if kazna_min is None else kazna_min
            )
            new_transapp_percent = (
                float(current_rate.transapp_percent)
                if transapp_percent is None
                else transapp_percent
            )
            new_transapp_min = (
                current_rate.transapp_min_amount
                if transapp_min is None
                else transapp_min
            )
            ...
            if kazna_percent is not None:
                new_kazna_percent = kazna_percent
            if kazna_min is not None:
                new_kazna_min = kazna_min
            if transapp_percent is not None:
                new_transapp_percent = transapp_percent
            if transapp_min is not None:
                new_transapp_min = transapp_min
