...
- **2026-04-21**: DriverListScreen.web.tsx fixed: removed incorrect `<WebAppLayout>` wrap (same bug class as notifications — authenticated layout already provides it), added `<ScreenHeader>` + `<WebScreenContainer maxWidth=960>` + `safeBack`. Migrated to NativeWind. Delegates all CRUD to existing `DriversTab` (reused both here and inside AutoDetail). Follow-up task flagged: mobile counterpart is a 533-LOC class component duplicating DriversTab logic — separate refactor needed to extract `useDriverList` hook + shared `DriverCard`/`DriverEditModal`/`DriverDeleteModal` sub-components and convert mobile to functional.
