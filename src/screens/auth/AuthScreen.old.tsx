...
interface AuthScreenState {
  ...
  modalUserAgreement: boolean;
  privacy_policy: string;
  modalPrivacyPlolicy: boolean;
  modalWaitConfirmation: boolean;
  session_data: any;
}
...
class AuthScreen extends Component<AuthScreenProps, AuthScreenState> {
  ...
  setButtonStyle = () => {
    ...
    return { height: 50, width: 275, borderRadius: 5, alignItems: 'center' as const, justifyContent: 'center' as const, backgroundColor: backgroundColor }
  }
            ...
            data.session_data.user_data.user_confirmed === 0)
        {
          ...
          let get_session = setInterval(async () => {
            ...
            try {
              ...
              if(error.response?.status === 401) { clearInterval(get_session); }
            }
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const data = res.data;
      this.setState({ 
        user_agreement: data.content_data.user_agreement, 
        privacy_policy: data.content_data.privacy_policy 
      });
    } catch (error) {
      console.log('error');
      console.log(error);
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity
            ...
            onPress={async () => {
              ...
              try {
                const res = await api.post('/auth-by-phone', { phone: phone });
                const data = res.data;
                console.log(data);
