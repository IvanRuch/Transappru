...
interface OurServicesData {
  fio: string;
  phone: string;
  leasing_subject: string;
  auto_quantity: string;
  waybill_quantity: string;
  permission_quantity: string;
  spb_pass_quantity: string;
  tow_truck_quantity: string;
  map_quantity: string;
  leasing_selection: string;
  leasing_time: string;
  leasing_advance: string;
  insurance_type: string;
  permission_international: string;
  ais_ossig_region: string;
  fine_reason: string;
  comment: string;
...
interface OurServicesState {
  service_data: any;
  our_services_data: OurServicesData;
  modalServiceOrderButtonDisabled: boolean;
  modalWaitForConnection: boolean;
}

class OurServicesClass extends React.Component<OurServicesProps, OurServicesState> {
  ...
  constructor(props: OurServicesProps) {
    ...
    let service_data = {};
    try {
      if (props.route?.params?.service_data) {
        service_data = JSON.parse(props.route.params.service_data);
      }
    } catch (e) {
      console.log('Error parsing service_data:', e);
    }
  ...
  setOurServicesOrderTextStyle = () => {
    ...
    return { fontSize: 24, fontWeight: '500' as const, color }
  }

  setTextInputStyle = (field: string) => {

    let color = "#656565", bgcolor = "#FFFFFF";
...
const OurServices = () => {
  ...
  return (
    <OurServicesClass
      route={{
        params: {
          service_data: params.service_data as string || '{}'
        }
      }}
      navigation={{
